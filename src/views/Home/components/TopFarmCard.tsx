...
const TopFarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethereum, account }) => {
  ...
  return (
    <div className="topfarmcardbg mx-1 xs:mx-1 sm:mx-2 rounded-3xl mb-4 shadow-sm p-5 flex flex-col items-center justify-around"style={{  maxWidth:260}}>
      <div className="self-end mt-6 mr-4">
        <div className="bg-red-600 text-white px-6 py-1 text-sm text-center rounded-3xl">Hot</div>
      </div>
      <Image
        ...
        marginBottom="15px"
      />
      <div className="font-bold text-white text-md mb-2">{farm.lpSymbol}</div>
      ...
      <Link className="bg-gray-300 px-7 py-2 rounded-3xl text-primary cursor-pointer hover:opacity-75 mt-4" to="/farms">
        Farm
      </Link>
    </div>
  )
